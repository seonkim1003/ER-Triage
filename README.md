...
- `tests/test_pipeline.py`: leakage, split, validation and policy regression tests.
- `artifacts/`: local run outputs; `data/`: local records and download provenance.

Only load this project's trusted local `.joblib` files: the serialization format can execute code. Future work should add site-held-out evaluation, patient-bootstrap uncertainty, calibration assessment, official utility scoring, clinician-designed policies and genuinely ER-specific retrospective validation.
